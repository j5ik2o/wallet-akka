...
import wallet.adaptor.typed.WalletProtocol.{ WalletCharged, WalletCreated, WalletDeposited, WalletWithdrawed }
import wallet.adaptor.typed.json.{ WalletChargedJson, WalletCreatedJson, WalletDepositedJson, WalletWithdrawedJson }
...
class WalletEventJsonSerializer(system: ExtendedActorSystem) extends SerializerWithStringManifest {
  ...
  import wallet.adaptor.typed.json.WalletWithdrawedJson._
  ...
  final val WalletWithdrawedManifest = classOf[WalletWithdrawed].getName
